...
# Configure logging with force flush for Windows
logging.basicConfig(
    level=logging.INFO,
...
class FileDropHandler(FileSystemEventHandler):
    ...
    def __init__(self, vault_path: str, logger: logging.Logger):
        ...
        self.pending_files: dict = {}  # Track files being copied
    ...
    def is_file_ready(self, file_path: Path) -> bool:
        ...
        try:
            # Try to open the file - if it fails, file is still being written
    ...
    def process_file(self, file_path: Path):
        """Process a file and create action file."""
        # Skip if already processed
        file_key = f"{file_path.name}_{int(file_path.stat().st_mtime)}"
        ...
        # Skip action files created by this watcher (prevent infinite loops)
        # Action files are in Needs_Action folder and have the FILE_ prefix
        if file_path.suffix == '.md' and file_path.name.startswith('FILE_'):
            return

        # Check if file is ready (fully copied)
        if not self.is_file_ready(file_path):
            # File still being copied, retry later
            self.pending_files[file_path.name] = file_path
            return
    ...
    def on_modified(self, event):
        """Called when a file is modified (catches copied files)."""
        if event.is_directory:
            return

        file_path = Path(event.src_path)
        
        # Check if this is a new file that was just copied
        file_key = f"{file_path.name}_{int(file_path.stat().st_mtime)}"
...
class FileSystemWatcher(BaseWatcher):
    ...
    def run(self):
        ...
        # Set up the observer with daemon=True for background operation
        ...
        # Scan for existing files (catch files copied before watcher started)
        ...
        try:
            while True:
                time.sleep(1)
                # Periodically retry pending files (files that were being copied)
                if self.handler.pending_files:
    ...
    def scan_for_existing_files(self):
        """Scan for files that exist before watcher started."""
        print("Scanning for existing files...")
        for file_path in self.drop_folder.iterdir():
            if file_path.is_file():
                file_key = f"{file_path.name}_{int(file_path.stat().st_mtime)}"
                if file_key not in self.handler.processed_files:
                    self.handler.process_file(file_path)
        print("Scan complete.\n")
    
    def retry_pending_files(self):
        """Retry processing files that were being copied."""
        files_to_remove = []
        for name, file_path in self.handler.pending_files.items():
            if file_path.exists():
                self.handler.process_file(file_path)
                files_to_remove.append(name)
        for name in files_to_remove:
            del self.handler.pending_files[name]
